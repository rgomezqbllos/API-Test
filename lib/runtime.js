...
function deriveBaseUrl(defaults, envConf) {
...
export async function resolveRuntime({
    ...
    if (!config || !config.queries) {
        throw new Error(`No se encontró la configuración de consultas en ${CONFIG_FILE}.`);
    }
    const queries = ensureArray(config.queries);
    const defaults = ensureObject(config.defaults);
    ...
    const baseUrl = deriveBaseUrl(defaults, envConf);
    if (!baseUrl) {
    ...
    if (envConf && envConf.auth_mode === 'client_credentials') {
        ...
        const identityBaseUrl = envConf.identity_base_url;
        const realm = envConf.realm;
        const username = envConf.username;
        const password = envConf.password_env ? envVars[envConf.password_env] : envConf.password;
        const clientId = envConf.client_id;
        ...
        throw new Error('No se pudo determinar la configuración de autenticación para el ambiente seleccionado.');
    }
